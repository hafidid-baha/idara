							...
							<div class="d-flex">
								<div class="dropdown">
									...
									<div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
										...
										<a class="dropdown-item text-right" href="<?= site_url('/admin/index');?>">
											لوحة التحكم
										</a>
				...
				<div class="vobilet-navbar" id="headerMenuCollapse">
					<div class="container">
						<ul class="nav" style="direction: rtl;">
							<li class="nav-item">
								<a class="nav-link <?=$page=='posts'?'active':'';?>" href="<?=site_url('blog/posts');?>">
									<span>الرئيسية</span>
								</a>
							</li>
							<?php foreach($postTypes as $type): 
								$subTypes = $this->postType_model->getAllSubPostType($type->id);
								?>
								<li class="nav-item">
									<?php if(empty($subTypes)): ?>
									<a class="nav-link <?=$page==$type->id?'active':''?>" href="<?=site_url('blog/posts/'.$type->id);?>">
										<span><?=$type->name?></span>
									</a>
									<?php else:
										$subIds = [];
										...
										<li class="nav-item with-sub ">
											<a class="nav-link <?=in_array($page,$subIds) ?'active':''?>" data-toggle="dropdown" href=""><span><?=$type->name;?></span></a>
											<div class="sub-item">
												<ul>
													<?php foreach($subTypes as $sub): 
														?>
														<li>
															<a href="<?=site_url('blog/posts/'.$sub->id);?>"><?=$sub->name ?></a>
														</li>
													<?php endforeach; ?>
												</ul>
											</div>
